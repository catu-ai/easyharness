...
1. Decide the next release version, such as `0.2.1`, and update the
...
5. Confirm the `Tag Release From VERSION` workflow created the matching git
   tag, for example `v0.2.1`, and then dispatched the `Release`
...
`VERSION=0.2.1` maps to the release tag `v0.2.1`.
