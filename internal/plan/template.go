...
func renderGoalOrientedTemplate(rendered string, size string) string {
	...
	rendered = strings.Replace(rendered, "### Out of Scope\n\n- Item", "### Out of Scope\n\n- Full status next-action support, archive/reopen profile preservation, automatic challenge prompting, UI rendering, and structural lint enforcement.\n- A hypothesis state machine or separate workflow engine.\n- Changes to default standard or lightweight behavior.", 1)
	rendered = strings.Replace(rendered, "- [ ] Criterion 1\n- [ ] Criterion 2", goalOrientedAcceptanceCriteria, 1)
	rendered = strings.Replace(rendered, "- None.", "- Full execution support remains in the v0.6.0 follow-up implementation slices for status next actions, structural lint coverage, docs/examples, archive/reopen behavior, and UI rendering.", 1)
